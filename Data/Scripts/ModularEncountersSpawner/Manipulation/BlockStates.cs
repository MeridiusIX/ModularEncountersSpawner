...
namespace ModularEncountersSpawner.Manipulation {
	public static class BlockStates {
		...
		public static void ProcessDereliction(MyObjectBuilder_CubeGrid grid, ImprovedSpawnGroup spawnGroup) {

			if (grid?.CubeBlocks == null)
				return;

			foreach (var profileName in spawnGroup.DerelictionProfiles) {

				DerelictionProfile profile = null;

				if (!DerelictionProfiles.TryGetValue(profileName, out profile))
					continue;
